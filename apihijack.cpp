...
bool RedirectIAT( SDLLHook* DLLHook, PIMAGE_IMPORT_DESCRIPTOR pImportDesc, PVOID pBaseLoadAddr )
{
        ...
        if ( !IMAGE_SNAP_BY_ORDINAL( pINT->u1.Ordinal ) )  // import by name
        {
            ...
            while ( FHook->Name )
            {
                ...
                if ( lstrcmpi( FHook->Name, (char*)pImportName->Name ) == 0 )
                {
					...
					sprintf(szDebug, "Hooked function: %s %p->%p\n", (char*)pImportName->Name, FHook->OrigFn, FHook->HookFn);
	...
	OutputDebugString("RedirectIAT end\n");
    return true;
}
...
bool HookAPICalls( SDLLHook* Hook )
{
	...
	PSTR pszLastModuleName = NULL;

    while ( pImportDesc->FirstThunk )
    {
        PSTR pszImportModuleName = MakePtr( PSTR, hModEXE, pImportDesc->Name);
		PSTR lastDllName = NULL;

		for (c=0; Hook->Functions[c].dllName; c++) {
			if (lastDllName != Hook->Functions[c].dllName) {
				lastDllName = Hook->Functions[c].dllName;
...
bool UnhookAPICalls( SDLLHook* Hook )
{
	...
	for (int c=0; Hook->Functions[c].dllName; c++) {
		sprintf(szDebug, "UnhookAPICalls: %s %s %p->%p\n", Hook->Functions[c].dllName,  Hook->Functions[c].Name, Hook->Functions[c].HookFn, Hook->Functions[c].OrigFn);
		OutputDebugString(szDebug);
		Hook->Functions[c].HookFn = Hook->Functions[c].OrigFn;
	}

	return HookAPICalls(Hook);
}